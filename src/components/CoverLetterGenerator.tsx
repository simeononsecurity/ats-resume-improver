    ...
    try {
      ...
      if (resumeData.name) {
        children.push(new Paragraph({
          children: [new TextRun({ text: resumeData.name, bold: true, size: 36, color: NAVY, font: 'Calibri' })],
          spacing: { after: 60 },
        }))
        ...
        // Accent rule
        children.push(new Paragraph({
          border: { bottom: { style: BorderStyle.THICK, size: 18, color: BLUE } },
          spacing: { after: 200 },
        }))
      }
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: dateStr, size: 20, color: GRAY, font: 'Calibri', italics: true })],
        spacing: { after: 240 },
      }))

      // ── Letter body: split into paragraphs ──
      const letterParagraphs = coverLetter.split('\n')
      for (const line of letterParagraphs) {
        ...
        children.push(new Paragraph({
          ...
          spacing: { after: isEmpty ? 80 : 180, line: 288 },
        }))
      }
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: 'Generated by ATS Resume Match', size: 16, color: RULE, font: 'Calibri', italics: true })],
      }))
